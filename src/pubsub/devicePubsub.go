...
// In WebSocketHub
func (hub *WebSocketHub) GetAndClearDeviceAction(referenceId string, deviceID int64) (map[string]any, error) {
	hub.Mu.Lock()
	defer hub.Mu.Unlock()
	...
	for _, client := range hub.Devices {
		if client.DeviceID == deviceID {
			...
			client.Action = nil // Clear the action
			return action, nil
		}
	}
...
func (hub *WebSocketHub) SetDeviceAction(referenceId string, deviceId int64, action map[string]any) error {
	...
	logger.Debug(referenceId, fmt.Sprintf("DEBUG - SetDeviceAction - Device ID: %d, Action: %v", deviceId, action))
	...
	if conn, exists := hub.DeviceConn[deviceId]; exists {
		...
		// Only set the action if it's not empty
		if len(action) > 0 {
...
func (hub *WebSocketHub) RestartDevice(referenceId string, deviceId int64) error {
	...
	device.Action = map[string]any{"type": "restart", "device_id": deviceId}

	logger.Info(referenceId, fmt.Sprintf("INFO - RestartDevice - Set restart action for device ID %d", deviceId))
	return nil
}

// DeepSleepDevice mengirimkan perintah deep_sleep ke device melalui WebSocket
func (hub *WebSocketHub) DeepSleepDevice(referenceId string, deviceId int64) error {
	hub.Mu.Lock()
	defer hub.Mu.Unlock()
	...
	device := hub.Devices[conn]
	device.Action = map[string]any{"type": "deep_sleep", "device_id": deviceId}

	logger.Info(referenceId, fmt.Sprintf("INFO - DeepSleepDevice - Set deep sleep action for device ID %d", deviceId))
	return nil
}
