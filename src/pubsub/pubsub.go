...
/*
	...
	each user can subscribe to multiple channel at a time
...
type WebSocketHub struct {
	...
	ChannelUsers map[string]map[*websocket.Conn]bool // <- GANTI jadi map[*websocket.Conn]bool
	redis        *redis.Client
}
...
type UserClient struct {
	...
	PubSubCancels      map[string]context.CancelFunc
}
...
func NewWebSocketHub(referenceId string) (*WebSocketHub, error) {
	...
	hub := &WebSocketHub{
		...
		ChannelUsers: make(map[string]map[*websocket.Conn]bool),
		redis:        redisClient,
	}
